...
output_video = cv2.VideoWriter(
    output_path, fourcc, desired_frame_rate, (640, 480))
vid = cv2.VideoCapture('wheel.mp4')
...
# This next section is just for visualization, we may want to put this in a separate function to easily turn it on/off for testing, we will get better performance without it on
        ...
        cv2.putText(image_show, f"{name}, {class_name}",
                    (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 1)
...
# If no motion, upload data
# If no detections and counts in class dict, upload
# store state, if no change in state and no motion, don't upload